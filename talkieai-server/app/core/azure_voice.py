...
def speech_default(content: str, output_path_str: str, language: str, voice_name: str|None = None):
    """默认语音合成 - 模拟版本"""
    logging.info(f"语音合成已禁用，创建空音频文件: {content}")
    # 创建一个空的 WAV 文件作为占位符
...
def speech_by_ssml(content: str, output_path_str: str, voice_name: str, speech_rate: str, feel: str, targetLang: str):
    """可定制的文本转语音 - 模拟版本"""
    logging.info(f"SSML语音合成已禁用: {content}")
    # 创建一个空的 WAV 文件作为占位符
    with open(output_path_str, 'wb') as f:
        f.write(b'RIFF$\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00D\xac\x00\x00\x88X\x01\x00\x02\x00\x10\x00data\x00\x00\x00\x00')
...
def speech_pronunciation(content: str, speech_path: str, language: str = "en-US"):
    """发音评估 - 模拟版本"""
    logging.info(f"发音评估已禁用: {content}")
...
def get_azure_voice_role_by_short_name(short_name: str):
    """根据short_name获取语音配置 - 模拟版本"""
    return {
        "gender": 1,
        "locale": "en-US",
        "local_name": "Default Voice",
        "name": "Microsoft Server Speech Text to Speech Voice (en-US, Default)",
        "short_name": "en-US-DefaultNeural",
        "voice_type": {
            "name": "OnlineNeural",
            "value": 1
        },

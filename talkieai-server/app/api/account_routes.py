...
@router.post("/account/login", name="User Login")
def login(
    request: Request, dto: LoginDTO, db: Session = Depends(get_db)
):
    """用户账号密码登录"""
    if not dto.username or not dto.password:
        return ApiResponse(code="400", status="FAILED", message="用户名或密码不能为空")
    
    account_service = AccountService(db)
    try:
        token = account_service.login(dto.username, dto.password)
        return ApiResponse(data=token)
    except Exception as e:
        return ApiResponse(code="401", status="FAILED", message=str(e))

@router.post("/account/register", name="User Register")
def register(
    request: Request, dto: RegisterDTO, db: Session = Depends(get_db)
):
    """用户注册"""
    if not dto.username or not dto.password:
        return ApiResponse(code="400", status="FAILED", message="用户名或密码不能为空")
    ...
    if len(dto.password) < 6:
        return ApiResponse(code="400", status="FAILED", message="密码长度不能少于6位")
    
    account_service = AccountService(db)
    try:
        token = account_service.register(dto.username, dto.password)
        return ApiResponse(data=token)
    except Exception as e:
        return ApiResponse(code="400", status="FAILED", message=str(e))

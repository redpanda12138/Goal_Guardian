...
from app.core.exceptions import AIServiceQuotaExceededException

try:
    from zhipuai.core._errors import APIReachLimitError
except ImportError:
    APIReachLimitError = None  # type: ignore
...
class ZhipuAIComponent(ChatAI):
    ...
    def _original_invoke_chat(self, dto: MessageInvokeDTO):
        ...
        try:
            ...
            if APIReachLimitError is not None and isinstance(e, APIReachLimitError):
                raise AIServiceQuotaExceededException(
                    "智谱 API 额度不足或无可用资源包，请登录 https://open.bigmodel.cn 充值或更换 API Key。"
                ) from e
            raise

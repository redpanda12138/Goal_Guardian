...
class SysService:
    ...
    def voice_upload(self, file: UploadFile, account_id: str):
        ...
        logging.info(f"语音上传: {file_name}, 原始大小: {file_size} bytes")
        
        if file_size == 0:
            logging.error(f"上传的语音文件为空: {file_name}")
            raise ValueError(f"上传的语音文件为空，请检查录音是否成功")
        
        # 如果上传的是mp3格式(暂时只有android手机只能用mp3格式), 就转换成wav返回, 为了后续azure服务解析音频(mp3会解析失败), 因为chat接口本身比较慢，所以在这里进行转换
        if file.filename.endswith(".mp3"):
            mp3_file_path = voice_file_get_path(file_name)
            wav_file_name = file_name.replace(".mp3", ".wav")
            wav_file_path = voice_file_get_path(wav_file_name)
            sound = AudioSegment.from_mp3(mp3_file_path)
            sound.export(wav_file_path, format="wav")
            wav_size = os.path.getsize(wav_file_path)
            logging.info(f"MP3转WAV完成: {wav_file_name}, 大小: {wav_size} bytes")
            # mp3文件需要删除

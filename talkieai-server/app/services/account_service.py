...
class AccountService:
    ...
    def login(self, username: str, password: str):
        """用户名密码登录"""
        # 查找用户
        account = self.db.query(AccountEntity).filter_by(username=username).first()
        if not account:
            raise UserAccessDeniedException("用户不存在")
            
        # 验证密码
        if not account.password or not pwd_context.verify(password, account.password):
            raise UserAccessDeniedException("密码错误")
            
        self.__check_and_init_default_settings(account.id)
        return auth.init_token(account.id, account.id)
        
    def register(self, username: str, password: str):
        """用户注册"""
        # 检查用户名是否已存在
        existing_account = self.db.query(AccountEntity).filter_by(username=username).first()
        if existing_account:
            raise UserAccessDeniedException("用户名已存在")
        ...
        self.__check_and_init_default_settings(new_account.id)
        return auth.init_token(new_account.id, new_account.id)

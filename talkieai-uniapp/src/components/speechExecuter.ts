...
class Speech {
      ...
      (msg: string, isUserNotAllow: any) => {
        ...
        if(isUserNotAllow) {
          uni.showModal({
            ...
            content: 'Microphone permission has been denied. Please allow microphone access and refresh the page.',
            showCancel: true,
            ...
            success: (modalRes) => {
              if (modalRes.confirm) {
                uni.showToast({
                  title: 'Please enable microphone permission in browser settings',
                  icon: 'none',
                  duration: 3000
                });
              }
          ...
          uni.showToast({
            title: "Please enable recording permission",
            icon: "none",
          });
        }
        if (self.listener.error) {
          // 传递更多信息给调用方
          self.listener.error(new Error(`Recording permission error: ${msg}`));
        }
      }
    );
  }
  ...
  handleH5EndVoice() {
    ...
    self.recorder.rec.stop(
      (blob: any, duration: any) => {
        self.recorder.processing = true;
        if (!blob || blob.size === 0) {
          console.error("录音 Blob 为空，无法上传");
          if (self.listener.error) self.listener.error(new Error("录音数据为空"));
          self.recorder.start = false;
          self.recorder.processing = false;
          self.recorder.rec = null;
          return;
        }
        // 将 Blob 转为 File 并指定文件名，确保 H5 上传时服务端能正确接收（避免 0 字节）
  ...
  handleUploadResult({ resData }: { resData: any }) {
    ...
    if (resData.statusCode == 200) {
      ...
      console.error("Upload failed with status: ", resData.statusCode);
      if (self.listener.error) {
        self.listener.error(new Error(`Upload failed: ${resData.statusCode}`));
      }
    }
  }
}
